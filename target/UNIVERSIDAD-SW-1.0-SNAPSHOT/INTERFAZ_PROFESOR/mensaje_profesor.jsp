...
<%
    ...
    try {
        ...
        try {
            ...
            if (rs.next()) totalPendingJoinRequests = rs.getInt(1);
            ...
            pstmt.setInt(1, idProfesor);
            rs = pstmt.executeQuery();
            if (rs.next()) totalPendingLeaveRequests = rs.getInt(1);
            closeDbResources(rs, pstmt);
            ...
            pstmt.setInt(1, idProfesor);
            rs = pstmt.executeQuery();
            if (rs.next()) totalUnreadMessages = rs.getInt(1);
            closeDbResources(rs, pstmt);

            // Count total sent messages by this professor
            String sqlTotalSent = "SELECT COUNT(DISTINCT m.id_mensaje) FROM mensajes m " +
                                  "INNER JOIN inscripciones i ON m.id_destinatario = i.id_alumno " + // Assuming messages sent by professor are to students through inscriptions
            ...
            rs = pstmt.executeQuery();
            if (rs.next()) totalSentMessages = rs.getInt(1);
            closeDbResources(rs, pstmt);
            ...
            pstmt.setInt(1, idProfesor);
            rs = pstmt.executeQuery();
            if (rs.next()) totalReceivedMessages = rs.getInt(1);
            closeDbResources(rs, pstmt);
            ...
            pstmt.setInt(1, idProfesor);
            rs = pstmt.executeQuery();
            if (rs.next()) totalActiveClasses = rs.getInt(1);
            closeDbResources(rs, pstmt);
...
<head>
    ...
    <style>
        ...
        .stat-card.messages-sent .icon-wrapper { background-color: var(--admin-primary); } /* Blue */
        .stat-card.messages-received .icon-wrapper { background-color: var(--admin-info); } /* Cyan */
        .stat-card.active-classes .icon-wrapper { background-color: var(--admin-success); } /* Green */
        ...
        .badge-info-custom { background-color: rgba(23, 162, 184, 0.1); color: var(--admin-info); border: 1px solid var(--admin-info); }
        .badge-read-custom { background-color: rgba(108, 117, 125, 0.1); color: var(--admin-secondary-color); border: 1px solid var(--admin-secondary-color); }
        .badge-unread-custom { background-color: rgba(0, 123, 255, 0.1); color: var(--admin-primary); border: 1px solid var(--admin-primary); }
        ...
        @media (max-width: 768px) {
            ...
            .stat-card { margin-bottom: 1rem; } /* Add margin for stacking on smaller screens */
        }
        ...
        @media (max-width: 576px) {
            ...
            .row-cols-md-2 > .col { flex: 0 0 100%; max-width: 100%; } /* Force single column on smaller phones for cards */
        }
    </style>
</head>
<body>
                                    ...
                                    <table class="table table-hover table-sm">
                                        ...
                                        <tbody>
                                            <%
                                                ...
                                                try {
                                                    if (conn == null || conn.isClosed()) {
                                                        conn = new Conexion().conecta(); // Reopen if closed
                                                    }
                                            ...
                                            <tr class="<%= !leido ? "table-primary-subtle" : "" %>"> <%-- Highlight unread messages --%>
                                                ...
                                                <td class="text-center">
                                                    <% if (!leido) { %>
                                                    <form action="<%= request.getContextPath() %>/marcar_leido_mensaje.jsp" method="post" class="d-inline-block me-1">
                                                        <input type="hidden" name="id_mensaje" value="<%= idMensaje %>">
                                                        <input type="hidden" name="redirect_url" value="<%= request.getContextPath() %>/INTERFAZ_PROFESOR/mensaje_profesor.jsp">
        ...
        $(document).ready(function() {
            ...
            function animateValue(id, start, end, duration) {
                ...
                function step() {
                    ...
                    if (current != end) setTimeout(step, stepTime);
                }
                obj.textContent = start;
                if (start !== end) step(); // Only animate if start and end are different
            }
